...
const getImpactCardWrapper = (impactCard, placeholdersData) => {
  const [indicatorTag, icon, title, description, cardLink, borderColor] = [...impactCard.children];
  const colorCode = borderColor ? borderColor.textContent : '';
  const impactCardWrapper = div({ class: `impact-card-wrapper ${colorCode}` });
