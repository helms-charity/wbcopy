...
export async function fetchSearch() {
  ...
  if (Object.keys(window.searchData).length === 0) {
    const path = `${LANGUAGE_ROOT === '/ext/ja' ? '/ext/en' : LANGUAGE_ROOT}/query-index.json?limit=500&offset=0`;
...
async function loadPage() {
  ...
  targetDiv.id = 'wbg-aem-target';
  targetDiv.className = 'wbg-aem-target';
  window.wbgData ||= {};
  await loadEager(document);
  await loadLazy(document);
  loadDelayed();
  document.body.insertAdjacentElement('afterbegin', targetDiv);
}
