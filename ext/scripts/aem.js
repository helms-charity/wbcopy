...
function setup() {
  ...
  window.hlx.codeBasePath = '/ext';
  window.hlx.lighthouse = new URLSearchParams(window.location.search).get('lighthouse') === 'on';

  const scriptEl = document.querySelector('script[src$="/ext/scripts/scripts.js"]');
  if (scriptEl) {
    try {
...
function init() {
  ...
  if (isAuthoring()) {
    // load author css
    loadCSSFile(`${window.hlx.codeBasePath}/styles/author.css`);
  }
  sampleRUM('top');
...
async function decorateGrid(main) {
  const sections = [...main.querySelectorAll(':scope > div.grid-section-container')];

  sections.forEach((section) => {
    const colCtrl = section.dataset.colCtrl || 'col-1';
    const colCount = parseInt(colCtrl.split('-')[1], 10);
    ...
    for (let i = 1; i <= colCount; i += 1) {
      ...
      // Add extra class from data-colX-class (if present)
      const extraClass = section.dataset[`col${i}Class`];
      if (extraClass && extraClass.trim()) {
        extraClass.split(' ').forEach((cls) => {
          if (cls.trim()) col.classList.add(cls.trim());
        });
      } else {
    ...
    // Track how many times a block type has been seen
    const blockCounters = {};
    ...
    children.forEach((child) => {
      ...
      if (!blockName || !blockToCols[blockName]) {
        // Not a managed block: leave in place
        section.insertBefore(child, section.lastElementChild?.nextSibling);
        return;
      }

      // How many times have we seen this block so far?
      const count = blockCounters[blockName] || 0;
      const allowedCols = blockToCols[blockName];

      let colIndex;
      if (allowedCols.length === 1) {
        // Only one possible column → always use it
        colIndex = allowedCols[0];
      } else {
        // Multiple possible columns → round-robin
        colIndex = allowedCols[count % allowedCols.length];
      }

      columns[colIndex].appendChild(child);
      blockCounters[blockName] = count + 1;
    });

    const rowWrapper = document.createElement('div');
    rowWrapper.classList.add('section-rows');

    // Append each column into the wrapper
    for (let i = 1; i <= colCount; i++) {
      rowWrapper.appendChild(columns[i]);
    }
